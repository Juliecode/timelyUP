...
PROMPT_WITH_PROFILE = """你是面向研发工程师的中文技术讲解人。请就下面这个主题，做一期适合**语音收听**的讲解。
...
PROMPT_GENERAL = """你是中文技术讲解人。请就下面这个主题，做一期客观、通用、适合**语音收听**的讲解（不针对任何特定行业或岗位）。
...
要求：
1. 口语化、有条理，像电台科普，目标时长约 {minutes} 分钟。不要出现 Markdown 符号、网址、列表符号、表情。
2. 结构建议：开场点题 → 它是什么/核心原理 → 为什么重要/现状/关键点 → 一句话小结。
3. 客观准确；不确定处说明是趋势或推测，不要编造具体数字或事实。

只输出一个 JSON 对象，字段：episode_title(简洁标题), spoken_script(完整讲解口播稿，纯文本), key_points(3-6 条要点的字符串数组)。不要输出任何额外文字。"""


def generate(topic: str, cfg: dict, use_profile: bool = True) -> dict:
    env = cfg["env"]
    ...
    if use_profile and profile:
        prompt = PROMPT_WITH_PROFILE.format(topic=topic, profile=profile, minutes=minutes)
    else:
        prompt = PROMPT_GENERAL.format(topic=topic, minutes=minutes)
...
def main() -> None:
    args = sys.argv[1:]
    use_profile = True
    if "--no-profile" in args:               # 本地命令行开关
        use_profile = False
        args = [a for a in args if a != "--no-profile"]
    topic = " ".join(args).strip() or os.environ.get("TOPIC", "").strip()
    if not topic:
        raise SystemExit('用法: python -m src.topic "你的话题" [--no-profile]')

    # CI 通过 USE_PROFILE 环境变量控制（优先于命令行）
    env_up = os.environ.get("USE_PROFILE", "").strip().lower()
    if env_up:
        use_profile = env_up in ("1", "true", "yes", "y", "on", "是")

    cfg = load_config()
    print(f"🎯 生成专题：{topic}（{'结合工作背景' if use_profile else '通用讲解'}）\n")

    data = generate(topic, cfg, use_profile=use_profile)
